...
import static com.spege.insanetweaks.InsaneTweaksMod.LOGGER;
...
public final class EffectTwistPairs {
    ...
    public static void install() {
        ...
        try {
            ...
            if (trait == null) {
                LOGGER.info("[InsaneTweaks] Effect Twist not present, skipping pair injection.");
                return;
            }
            ...
            if (!(raw instanceof Map)) {
                LOGGER.warn("[InsaneTweaks] Effect Twist's " + FIELD_NAME
            ...
            LOGGER.info("[InsaneTweaks] Effect Twist: added " + added + " effect pairs ("
                    + skipped + " skipped, mod absent), map now holds " + badPotions.size() + ".");
        } catch (NoSuchFieldException e) {
            LOGGER.warn("[InsaneTweaks] Effect Twist has no '" + FIELD_NAME
                    + "' field - Reskillable changed. Trait keeps its 6 vanilla pairs.");
        } catch (Exception e) {
            LOGGER.warn("[InsaneTweaks] Could not extend Effect Twist: " + e);
        }
    }
}
