...
import com.spege.reskilltweaks.customwizardrystats.SummonDurationStat;
...
public class EventHandlerSkills {
    ...
    private static java.lang.reflect.Field resolveInWebField() {
        ...
        com.spege.reskilltweaks.ReskillTweaks.LOGGER.error(
                "[ReskillTweaks] Could not resolve Entity.isInWeb - Spider's Grace will do nothing.");
        return null;
    }
    ...
    private static boolean isExplicitlyListedOre(ResourceLocation regName) {
        ...
        for (String extra : com.spege.reskilltweaks.config.ReskillTweaksConfig.traits.astralProspectorExtraOres) {
            if (id.equals(extra))
                return true;
        }
        return false;
    ...
    public void onLivingUpdate(LivingUpdateEvent event) {
        ...
        if (IN_WEB_FIELD != null && !spidersGraceFailed) {
            try {
                ...
                com.spege.reskilltweaks.ReskillTweaks.LOGGER.error(
                        "[ReskillTweaks] Spider's Grace failed and is disabled for this session.", e);
            }
        }
    }
