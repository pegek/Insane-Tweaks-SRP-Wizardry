...
public final class CurseReliefHelper {
    ...
    public static double reductionAtLevel(int level) {
        ...
        double reduction = level * TombTweaksConfig.tombstone.reliefForTheDamned.reductionPerLevel;
        if (reduction <= 0.0D) {
            return 0.0D;
        }
    ...
    public static double getReduction(EntityPlayer player) {
        if (player == null
                || !TombTweaksConfig.tombstone.enableTombstoneTweaks
                || !TombTweaksConfig.tombstone.reliefForTheDamned.enabled) {
            return 0.0D;
        }
