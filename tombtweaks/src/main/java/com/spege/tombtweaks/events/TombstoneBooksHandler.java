...
public class TombstoneBooksHandler {
    ...
    // The two cooldown NBT keys below still carry the InsaneTweaks_ prefix from before the R4
    // split. They live in the player's persisted data, so renaming them would hand every player a
    // free reset of both book cooldowns on the update.
    ...
    public void onPlayerLogin(PlayerEvent.PlayerLoggedInEvent event) {
        if (!com.spege.tombtweaks.config.TombTweaksConfig.tombstone.enableTombstoneTweaks || event.player.world.isRemote) {
            return;
        }
    ...
    public void onRightClickGrave(PlayerInteractEvent.RightClickBlock event) {
        if (!com.spege.tombtweaks.config.TombTweaksConfig.tombstone.enableTombstoneTweaks || event.getWorld().isRemote || event.getHand() != EnumHand.MAIN_HAND) {
            return;
        }
        ...
        if (regName.equals("tombstone:book_of_disenchantment")) {
            int conf = com.spege.tombtweaks.config.TombTweaksConfig.tombstone.bookOfDisenchantmentCooldownMinutes;
            if (conf <= 0) return;
            ...
            int conf = com.spege.tombtweaks.config.TombTweaksConfig.tombstone.bookOfMagicImpregnationCooldownMinutes;
            if (conf <= 0) return;
            cooldownMinutes = conf;
