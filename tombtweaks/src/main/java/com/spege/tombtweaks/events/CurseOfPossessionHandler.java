...
/**
 * Closes the Curse of Possession exploit: a cursed item is supposed to be destroyed on death, but
 * Tombstone puts the whole inventory in a grave first, so the curse never fires and the item comes
 * back with everything else.
 *
 * <p>Runs at {@code HIGHEST} priority, before the grave is built, and strips the cursed stacks out
 * of the inventory itself. What it takes is written to {@code CursedHistory} in the player's
 * persisted data so {@code /tombtweaks restore cursed} can hand it back if the removal was wrong —
 * the enchantment is matched by name fragments ("curse" + "posses"), which is deliberately loose
 * enough to catch every mod's spelling of it and therefore loose enough to over-match.
 *
 * <p>The NBT key is unchanged from when this lived in Insane Tweaks, so backups taken before the
 * split are still readable.
 */
public class CurseOfPossessionHandler {

    private void debugMessage(EntityPlayer player, String message) {
        if (!com.spege.tombtweaks.config.TombTweaksConfig.tombstone.curseOfPossessionDebugMessages)
            return;

        if (!player.world.isRemote) {
            player.sendMessage(new TextComponentString("\u00A7d[Tombstone Tweaks]\u00A7f " + message));
        }
    }
    ...
    public void onLivingDeath(LivingDeathEvent event) {
        if (!com.spege.tombtweaks.config.TombTweaksConfig.tombstone.enableTombstoneTweaks || !com.spege.tombtweaks.config.TombTweaksConfig.tombstone.enableCurseOfPossessionPatch) return;
